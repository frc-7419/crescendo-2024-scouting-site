...
const CurrentGame = ({eventName, loading, matches, time, shifts}: {
    eventName: string,
    loading: boolean,
    matches: Match[],
    time: Date,
    ...
    if (loading || isLoading) {
        return (
            <div
                className="dark:bg-slate-800 bg-slate-200 rounded-lg mt-6 mb-6 drop-shadow-lg shadow-inner flex flex-row">
                <div className='p-6 currentMatchCard'>
                    ...
                    <div className="nextShift text-2xl font-semibold align-bottom">Your next shift is <Spinner
                        color="default"/></div>
                </div>
                    ...
                    <div className="blueAllianceView bg-blue-600/60 h-20 rounded-b-lg items-center">
                        {isLoading ? (
                            ...
                            match && match.alliances.blue.team_keys.map((teamKey: string) => (
                                <div key={teamKey}
                                     className="flex-grow text-center text-2xl">{teamKey.replace("frc", "")}</div>
                            ))
                        )}
                    </div>
                    <div className="redAllianceView bg-red-600/60 h-20 rounded-t-lg items-center">
